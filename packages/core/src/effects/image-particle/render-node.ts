...
export class ImageParticleRenderNode {
  ...
  private async rebuild(): Promise<void> {
    ...
    try {
      ...
      // Defer CPU-heavy dithering to idle time — keeps the main thread free during scroll/load.
      // fixing-motion-performance: never block main thread with heavy compute during animation.
  ...
  /** Defers work to requestIdleCallback, falling back to setTimeout for Safari. */
  private runWhenIdle(task: () => void): Promise<void> {
    return new Promise((resolve) => {
      const run = () => { task(); resolve(); };
